...
TEST(correctness, andrey_test)
{
    ...
    std::vector<int> v{5, 2, 10, 6, 14, 7, 8};
...
TEST(correctness, find_empty)
{
    set<int> q;
    set<int>::const_iterator en(q.end());
    ASSERT_TRUE(en == q.find(17));
}
...
TEST(correctness, find)
{
    set<int> q;
    q.insert(17);
    set<int>::const_iterator be(q.begin());
    ASSERT_TRUE(be == q.find(17));
}
...
TEST(correctness, erase_return)
{
    set<int> q;
    q.insert(5);

    ASSERT_TRUE(q.erase(q.begin()) == q.end());
}
